...
data "archive_file" "lambda_terraform_test" {
  ...
  source_dir  = "${path.module}/../../out/retrieve-project"
  output_path = "${path.module}/retrieve-project.zip"
}

# to Create function
resource "aws_lambda_function" "terraform_test" {
  function_name    = "retrieve-project"
  filename         = "retrieve-project.zip"
  runtime          = "go1.x"
  handler          = "retrieve-project"
